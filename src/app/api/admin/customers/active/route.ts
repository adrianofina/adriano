...
export async function GET(request: Request) {
  try {
    ...
    const customers = await db.customer.findMany({
      ...
      include: {
        loans: {
          ...
          take: 1,
          include: {
            // Include the most recent payment so we can show lastPayment date
            payments: {
    ...
    const formatted = customers.map(c => {
      ...
      const progress = loan
        ? Math.round(((loan.amountPaid ?? 0) / (loan.amount || 1)) * 100)
        : 0;
      ...
      return {
        ...
        risk:          (c.creditScore ?? 0) >= 700 ? 'low'
                       : (c.creditScore ?? 0) >= 600 ? 'medium' : 'high',
      };
    });
    ...
    console.error('[GET /api/admin/customers/active]', error);
    return NextResponse.json(
